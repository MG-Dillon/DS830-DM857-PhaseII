...
class SimWindow:
    def __init__(
        self,
        segs: List[Tuple[Tuple[int], Tuple[int]]],
        cars: List[Tuple[int]],
        speed: List[Tuple[int]] = None,
    ...
    def _setup_themes(self):
        with dpg.theme() as global_theme:
            ...
            with dpg.theme_component(dpg.mvInputInt):
                dpg.add_theme_color(
                    dpg.mvThemeCol_FrameBg, (90, 90, 95), category=dpg.mvThemeCat_Core
                )
        ...
        with dpg.window(
            ...
            with dpg.collapsing_header(label="Camera Control", default_open=True):
                dpg.add_slider_float(
                    ...
                    callback=self._set_offset_zoom,
                )
                with dpg.group():
                    dpg.add_slider_float(
                        tag="OffsetXSlider",
    ...
    def _apply_transformation(self):
        screen_center = dpg.create_translation_matrix(
            [self._canvas_width / 2, self._canvas_height / 2, -0.01]
        )
    ...
    def _draw_segments(self):
        for segment in self.segments:
            dpg.draw_polyline(
                segment, color=(180, 180, 220), thickness=5 * self.zoom, parent="Canvas"
            )
            dpg.draw_circle(
                segment[0],
                2.5 * self.zoom,
